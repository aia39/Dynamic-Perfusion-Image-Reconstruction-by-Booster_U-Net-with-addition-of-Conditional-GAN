...
def r2c(inputs,dim='last'):
    if dim == 'last':
        ...
        outputs = inputs[:,0,...] * np.exp(1j * inputs[:,1,...])
    else:
        outputs = inputs[:,0,...] 
...
def c2r(inputs, dim='last'):
    if dim == 'last':
        ...
        outputs = np.zeros((inputs.shape[0],) + (2,) + inputs.shape[1:])
        outputs[:,0,...] = np.abs(inputs)
        outputs[:,1,...] = np.angle(inputs)
        
    return outputs.astype(np.float32)


def c2abs(inputs, dim='last'):
    if dim == 'first':
        outputs = np.zeros((inputs.shape[0],) + (1,) + inputs.shape[1:])
        outputs[:,0,...] = np.abs(inputs)
...
def check_orientation(x):
    x = np.transpose(x[2,:,:,:],(1,2,0))   #x = np.transpose(x[2,:,:,:],(1,2,0))
...
def validate_network(model,data_loader,loss_function,tracker,device):
    ...
    with torch.no_grad():
        for inputs,targets in data_loader:
            ...
            ######
            ##inputs = torch.permute(inputs, (0, 1, 4, 2, 3)).contiguous()
            outputs = model(inputs)
            ##outputs = torch.permute(outputs, (0, 1, 3, 4, 2)).contiguous()
            
            loss = loss_function(outputs,targets)
            ######## (see if I am using one or two channels image)
            tracker.calculate_metrics(outputs.detach().cpu().numpy(),targets.detach().cpu().numpy(),magnitude=False)
...
def test_network(model,data_loader,device):
    ...
    with torch.no_grad():
        ...
        for ins,tars in data_loader:
            ...
            #print(f"Size before model {ins.shape} and {tars.shape}")
            ##ins = torch.permute(ins, (0, 1, 4, 2, 3)).contiguous()
            outs = model(ins)
            
            ##outs = torch.permute(outs, (0, 1, 3, 4, 2)).contiguous()
            ##ins = torch.permute(ins, (0, 1, 3, 4, 2)).contiguous()
...
def blur_metric(original: np.ndarray) -> float:
    # Convert to float (like MATLAB's double)
    I = original.astype(float)
    y, x = I.shape

    # Vertical and horizontal averaging kernels
    Hv = np.ones((1, 9)) / 9.0
    Hh = Hv.T

    # Blur image vertically & horizontally
    B_Ver = convolve(I, Hv, mode='reflect')
    B_Hor = convolve(I, Hh, mode='reflect')

    # Variations of original
    D_F_Ver = np.abs(I[:, :x-1] - I[:, 1:x])      # vertical variation (diff along x-axis)
    D_F_Hor = np.abs(I[:y-1, :] - I[1:y, :])      # horizontal variation (diff along y-axis)

    # Variations of blurred
    D_B_Ver = np.abs(B_Ver[:, :x-1] - B_Ver[:, 1:x])
    D_B_Hor = np.abs(B_Hor[:y-1, :] - B_Hor[1:y, :])

    # Differences
    T_Ver = D_F_Ver - D_B_Ver
    T_Hor = D_F_Hor - D_B_Hor

    # Keep only positive values (max(0, T))
    V_Ver = np.maximum(0, T_Ver)
    V_Hor = np.maximum(0, T_Hor)

    # === Careful: indexing shift between MATLAB & Python ===
    # MATLAB: (2:y-1, 2:x-1) → excludes first & last row/col
    # Python: (1:y-1, 1:x-1)
    ...
    # Blur factors
    blur_F_Ver = (S_D_Ver - S_V_Ver) / (S_D_Ver + 1e-12)
    blur_F_Hor = (S_D_Hor - S_V_Hor) / (S_D_Hor + 1e-12)
...
class metric_tracker():
    ...
    def calculate_metrics(self,outputs,targets,magnitude=False):
        ...
        '''
        # For abs images
        nb,sx,sy,nt = tars.shape   #for abs images of 1 channel
        
        outs = np.transpose(outs,[0,3,1,2])
        tars = np.transpose(tars,[0,3,1,2])

        outs = np.reshape(outs,[nb*nt,sx,sy])
        tars = np.reshape(tars,[nb*nt,sx,sy])
        '''
        
        if(magnitude):
            nb,sx,sy,nt = tars.shape
            outs = np.squeeze(outputs)
            tars = np.squeeze(targets)
            ...
            # 2 channels complex
            nb,ch,sx,sy,nt = tars.shape   #for 2 channels complex
            ...
            nt,ch,sx,sy = tars.shape   #for 2 channels complex
            
            
            outs = np.abs(r2c(outs,dim='first'))
            tars = np.abs(r2c(tars,dim='first'))
            
            ## If we need mag phase
            #outs = np.abs(r2c(outs,dim='magphase'))
            #tars = np.abs(r2c(tars,dim='magphase'))
