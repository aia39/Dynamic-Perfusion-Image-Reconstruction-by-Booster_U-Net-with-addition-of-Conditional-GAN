...
class booster_runet(torch.nn.Module):
    def __init__(self,in_features, out_features, resolution, layers, N=16, dp=False, rl=False):
        ...
        self.unet1 = UNETRPP3D(img_size=(N, 144, 144),
                            patch_size=(2, 4, 4),
        ...
        self.unet2 = UNETRPP3D(img_size=(N, 144, 144),
                            patch_size=(2, 4, 4),
                            in_chans=2*in_features,
                            num_classes=out_features,
                            embed_dim=48,
                            encoder_depth=2,
                            num_heads=4)
    ...
    def forward(self,x):
        ...
        out1 = self.unet1(x)
        #print(f'out1 shape is {out1.shape} and x is {x.shape}')
